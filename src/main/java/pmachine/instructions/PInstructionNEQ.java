...
public class PInstructionNEQ extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionNEQ(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.NEQTypeIntBool"));
		}
	}

	protected void neqI(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemInt.neq(pm.getPMemory().pop(), sp1));
	}

	protected void neqB(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemBool.neq(pm.getPMemory().pop(), sp1));
	}
