...
public class PInstructionGEQ extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionGEQ(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.GEQTypeIntBool"));
		}
	}

	protected void geqI(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemInt.geq(pm.getPMemory().pop(), sp1));
	}

	protected void geqB(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemBool.geq(pm.getPMemory().pop(), sp1));
	}
}
