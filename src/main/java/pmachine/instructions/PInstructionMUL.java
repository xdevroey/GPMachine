...
public class PInstructionMUL extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionMUL(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation())
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.MULTypeIntAddr"));
		}
	}

	protected void mulN(PMachine pm) throws WrongTypeException, StackException {
		pm.getPMemory()
				.push(
						StackElemInt.mult(pm.getPMemory().pop(), pm
								.getPMemory().pop()));
	}

	protected void mulA(PMachine pm) throws WrongTypeException, StackException {
		pm.getPMemory().push(
				StackElemAddr
						.mult(pm.getPMemory().pop(), pm.getPMemory().pop()));
	}
