...
public class PInstructionSLI extends PTypedMemoryInstruction {

	/**
	 * @param type
	 */
	public PInstructionSLI(int type) {
		super(type);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isBooleanOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.SLITypeIntAddrBool"));
		}
	}

	protected void sliA(PMachine pm) throws WrongTypeException, StackException {
		StackElem s = pm.getPMemory().pop();
		if (s instanceof StackElemAddr) {
			pm.getPMemory().storeAt(pm.getSP() - 1, s);
		} else {
			throw new WrongTypeException(Messages
					.getString("PMachine.SLI_a_TypeError"));
		}
	}

	protected void sliB(PMachine pm) throws WrongTypeException, StackException {
		StackElem s = pm.getPMemory().pop();
		if (s instanceof StackElemBool) {
			pm.getPMemory().storeAt(pm.getSP() - 1, s);
		} else {
			throw new WrongTypeException(Messages
					.getString("PMachine.SLI_b_TypeError"));
		}
	}

	protected void sliI(PMachine pm) throws WrongTypeException, StackException {
		StackElem s = pm.getPMemory().pop();
		if (s instanceof StackElemInt) {
			pm.getPMemory().storeAt(pm.getSP() - 1, s);
		} else {
			throw new WrongTypeException(Messages
					.getString("PMachine.SLI_i_TypeError"));
		}
	}
