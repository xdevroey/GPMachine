...
public class PInstructionIXJ extends PInstructionAbstract {

	private String label;

	PInstructionIXJ(String label) {
		this.label = label;
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		// Jump ti Label label : ixj @label
		pm.jumpToLabel(label);
		// Add an offset that is on the top of stack (Int)
		StackElem sp = pm.getPMemory().pop();
		if (sp instanceof StackElemInt) {
			pm.getProgramMemory().setPC(
					pm.getProgramMemory().countInstructions(pm.getPC(),
							((StackElemInt) sp).longValue()));
		} else {
			throw new WrongTypeException(Messages
					.getString("PInstruction.IXJTypeTopInt"));
		}
	}
