...
public class PInstructionMSTF extends PTypedMemoryInstruction {

	private int p, q;

	public PInstructionMSTF(int p, int q) {
	...
	public void execute(PMachine pm) throws RuntimeException {
		...
		el = pm.getPMemory().elemAt(
				base(pm.getPMemory(), p, pm.getMP()).intValue() + q + 1);
		el.setCallCell(true);
