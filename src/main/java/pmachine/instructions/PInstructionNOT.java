...
public class PInstructionNOT extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionNOT(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isBooleanOperation()) {
			pm.getPMemory().push(StackElemBool.not(pm.getPMemory().pop()));
		} else {
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.NOTTypeBool"));
		}
	}
