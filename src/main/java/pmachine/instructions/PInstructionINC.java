...
public class PInstructionINC extends PTypedInstruction {
	 ...
	 * @param typeModifier
	 */
	public PInstructionINC(int typeModifier, int offset) {
		super(typeModifier);
		this.offset = offset;
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.INCTypeIntAddr"));
		}
	}

	protected void incI(PMachine pm) throws StackException, WrongTypeException {
		pm.getPMemory().push(
				StackElemInt.add(pm.getPMemory().pop(),
						new StackElemInt(offset)));
	}

	protected void incA(PMachine pm) throws StackException, WrongTypeException {
		pm.getPMemory().push(
				StackElemAddr.add(pm.getPMemory().pop(), new StackElemAddr(
						offset)));
	}
}
