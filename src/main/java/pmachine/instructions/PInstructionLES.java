...
public class PInstructionLES extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionLES(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.LESTypeIntBool"));
		}
	}

	protected void lesI(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemInt.lst(pm.getPMemory().pop(), sp1));
	}

	protected void lesB(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemBool.lst(pm.getPMemory().pop(), sp1));
	}
