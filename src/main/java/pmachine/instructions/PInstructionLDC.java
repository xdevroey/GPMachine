...
public class PInstructionLDC extends PTypedInstruction {
	 ...
	 * @param typeModifier
	 */
	public PInstructionLDC(int typeModifier, int value) {
		super(typeModifier);
		this.constant = value;
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isBooleanOperation()) {
			pm.getPMemory().push(new StackElemBool(constant));
		} else if (isIntegerOperation()) {
			pm.getPMemory().push(new StackElemInt(constant));
		} else if (isAddrOperation()) {
			pm.getPMemory().push(new StackElemAddr(constant));
		} else {
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.LDCTypeIntAddrBool"));
		}
	}
