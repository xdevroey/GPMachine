...
public class PInstructionGRT extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionGRT(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.GRTTypeIntBool"));
		}
	}

	protected void grtI(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemInt.grt(pm.getPMemory().pop(), sp1));
	}

	protected void grtB(PMachine pm) throws WrongTypeException, StackException {
		StackElem sp1 = pm.getPMemory().pop();
		pm.getPMemory().push(StackElemBool.grt(pm.getPMemory().pop(), sp1));
	}
}
