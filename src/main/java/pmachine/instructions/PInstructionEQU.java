...
public class PInstructionEQU extends PTypedInstruction {

	/**
	 * @param typeModifier
	 */
	public PInstructionEQU(int typeModifier) {
		super(typeModifier);
	}
	...
	public void execute(PMachine pm) throws RuntimeException {
		if (isIntegerOperation()) {
			...
			throw new InvalidPInstructionException(Messages
					.getString("PInstruction.EQUTypeIntBool"));
		}
	}

	protected void equI(PMachine pm) throws WrongTypeException, StackException {
		pm.getPMemory().push(
				StackElemInt.eq(pm.getPMemory().pop(), pm.getPMemory().pop()));
	}

	protected void equB(PMachine pm) throws WrongTypeException, StackException {
		pm.getPMemory().push(
				StackElemBool.eq(pm.getPMemory().pop(), pm.getPMemory().pop()));
	}
}
