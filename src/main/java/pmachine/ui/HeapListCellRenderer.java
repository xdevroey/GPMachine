 ...
 * @author ybo
 */
public class HeapListCellRenderer extends javax.swing.DefaultListCellRenderer {
	...
	public HeapListCellRenderer(StateController parent, Set watchs, PMachine pm) {
		this.parent = parent;
		this.watchs = watchs;
		this.pm = pm;
	}
	...
	private String getIndexRepresentation(int index) {
		String maxAddress = "" + pm.getMemorySize();
		String indexAsString = "" + (pm.getMemorySize() - index);
		if (index > pm.getMemorySize()) {
	...
	public Component getListCellRendererComponent(JList list, Object value,
		...
		if (watchs.contains(new Integer(pm.getMemorySize() - index))) {
			...
			if (parent.isHeapWatchTriggered()) {
				if (pm.getMemorySize() - index == parent.getHeapWatch())
